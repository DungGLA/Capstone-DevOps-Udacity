		...
		parallel {
			stage('Build Docker Image blue') {
				steps {
					withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'udacity-docker-hub', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD']]){
						sh '''
							docker build -t dunggla2304/udacity-capstone-docker-blue ./docker/blue/
						'''
					}
				}
			}
			stage('Build Docker Image green') {
				steps {
					withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'udacity-docker-hub', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD']]){
						sh '''
							docker build -t dunggla2304/udacity-capstone-docker-green ./docker/green/
						'''
					}
				}
			}
				...
				steps {
					withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'udacity-docker-hub', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD']]){
						sh '''
							...
							docker push dunggla2304/udacity-capstone-docker-blue
						'''
					}
				}
			}
			...
			stage('Push Image To Dockerhub green') {
				steps {
					withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'udacity-docker-hub', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD']]){
						sh '''
							docker login -u $DOCKER_USERNAME -p $DOCKER_PASSWORD
							docker push dunggla2304/udacity-capstone-docker-green
						'''
					}
				}
			}
		...
		stage('Set Current kubectl Context') {
			steps {
                    ...
                    sh '''
						...
						kubectl config use-context arn:aws:eks:us-east-1:672481775548:cluster/udacitycluster
					'''
                    }
